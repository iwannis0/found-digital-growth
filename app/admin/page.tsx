...
export default async function AdminPage() {
  ...
  const [[leadCount], [auditCount], newCount, qualifiedCount, meetingCount, proposalCount, wonCount, [wonRevenue], [pipelineValue], [overdueLeads], [overdueAudits], [latestLead], [latestAudit], recentAudits] = await Promise.all([
    ...
    db.select({ value: count() }).from(leads).where(and(eq(leads.status, "NEW"), lt(leads.createdAt, sql`now() - interval '2 days'`))),
    db.select({ value: count() }).from(auditRequests).where(and(eq(auditRequests.status, "NEW"), lt(auditRequests.createdAt, sql`now() - interval '2 days'`))),
  ...
  const latestUpdate = [latestLead?.updatedAt, latestAudit?.updatedAt].filter((value): value is Date => Boolean(value)).sort((a, b) => a.getTime() - b.getTime()).at(-1);
