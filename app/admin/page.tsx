...
export default async function AdminPage() {
  ...
  const [[leadCount], [auditCount], newCount, meetingCount, proposalCount, wonCount, [revenue], recentAudits] = await Promise.all([
    db.select({ value: count() }).from(leads),
    db.select({ value: count() }).from(auditRequests),
    countStatus("NEW"),
    countStatus("MEETING_BOOKED"),
    countStatus("PROPOSAL_SENT"),
    countStatus("WON"),
  ...
  const metrics = [["Total Requests", leadCount.value + auditCount.value], ["New Requests", newCount], ["Audits", auditCount.value], ["Meetings", meetingCount], ["Proposals", proposalCount], ["Won", wonCount], ["Lead Won Value", `€${Number(revenue.value).toLocaleString()}`]];
