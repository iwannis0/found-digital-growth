...
export default async function AdminPage() {
  ...
  return <main className="admin-page"><div className="admin-heading"><div><p>FOUND. CONTROL ROOM</p><h1>Dashboard</h1></div><span>Live pipeline</span></div><section className="admin-metrics">{metrics.map(([label, value]) => <article key={label}><span>{label}</span><strong>{value}</strong></article>)}</section><section className="admin-panel"><div className="admin-panel-heading"><div><p className="eyebrow">Latest audit requests</p><h2>Recently submitted</h2></div><Link className="text-link" href="/admin/audits">View all audits</Link></div>{recentAudits.length ? <div className="recent-audits">{recentAudits.map((audit)=><Link href="/admin/audits" key={audit.id}><span><strong>{audit.businessName}</strong><small>{audit.name} · {new Date(audit.createdAt).toLocaleDateString("en-GB")}</small></span><em>{audit.status.replaceAll("_"," ")}</em></Link>)}</div> : <p>No audit requests yet.</p>}</section><section className="admin-panel"><p className="eyebrow">Pipeline guide</p><h2>Lead → Audit → Discovery → Proposal → Deposit → Build → Launch → Grow</h2><p>Use Leads and Audits to update status, add private notes and export records for follow-up.</p></section></main>;
}
