  ...
  return (
    <main>
      <section className="hero">
        <div className="site-container hero-grid">
          <div className="hero-copy reveal-up">
            ...
            <p className="hero-lead">We build premium websites and digital growth systems designed to turn searches and visitors into enquiries, bookings and customers.</p>
        ...
        <div className="site-container capabilities" aria-label="Core capabilities">
          {['Web Design', 'Local SEO', 'Google Business', 'Analytics', 'Automation', 'Conversion'].map((item) => <span key={item}>{item}</span>)}
        </div>
      </section>

      <section className="problem-section section-pad">
        <div className="site-container">
          <SectionHeading eyebrow="The hidden cost" title="Is your website costing you customers?" body="A weak digital presence does not just look dated. It creates doubt, loses visibility and makes the next step harder." />
      ...
      <section className="services-section section-pad">
        <div className="site-container">
          <SectionHeading eyebrow="More than a website" title="A connected digital presence, built around growth." body="Start with the foundation you need today, then add visibility, measurement and automation as the business grows." />
      ...
      <section className="growth-model">
        <div className="site-container growth-inner">
          <p className="eyebrow">The FOUND. growth model</p>
          <div className="growth-track">
            {['Website', 'Maintenance', 'SEO', 'Ads', 'Automation', 'Content'].map((item, index) => <div key={item}><span>{index + 1}</span><strong>{item}</strong></div>)}
          </div>
          <p>Your website is the foundation. Every next step should make it easier to be discovered, trusted and chosen.</p>
        </div>
      </section>

      <section className="work-section section-pad">
        <div className="site-container">
          <div className="section-topline"><SectionHeading eyebrow="Selected directions" title="Built to look better. Designed to perform better." /><Link className="text-link" href="/work">See all concept work <ArrowUpRight /></Link></div>
          <div className="project-grid">
            {projects.map((project, index) => (
              <Link href={`/work/${project.slug}`} className={`project-card project-${project.tone}`} key={project.slug}>
                <div className="project-image-wrap">
                  <Image src={`/work/${project.slug}.png`} alt={`${project.name} concept project`} fill sizes="(max-width: 900px) 100vw, 50vw" className="project-image" />
