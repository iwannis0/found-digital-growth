...
export default async function ProjectPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  return <main className={`case-study case-${project.tone}`}><PageTracker event="project_view" label={project.name} />
    <section className="case-hero"><div className="site-container"><div className="case-kicker"><span>Concept Project</span><span>{project.industry}</span></div><h1>{project.name}</h1><p>{project.statement}</p></div></section>
    <section className="case-image"><div className="site-container"><Image src={`/work/${project.slug}.png`} alt={`${project.name} concept art direction`} width={1584} height={990} priority /></div></section>
    ...
    <section className="case-disclaimer"><div className="site-container"><strong>Concept Project</strong><p>This creative direction was developed internally by FOUND. It does not represent paid client work, performance results or an existing commercial relationship.</p></div></section>
