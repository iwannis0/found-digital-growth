...
export default function WorkPage() {
  return <main>
    ...
    <section className="business-types-section section-pad"><div className="site-container"><div className="business-types-intro"><p className="eyebrow">Built around your business</p><h2>What does your business<br /><em>need its website to do?</em></h2><p>Different businesses need different paths to action. We design around the way your customers discover you, evaluate you and choose what to do next.</p></div><div className="business-type-list">{businessTypes.map(([title, examples, capabilities], index) => <article key={title}><span>{String(index + 1).padStart(2, "0")}</span><div><h3>{title}</h3><p>{examples}</p><strong>{capabilities}</strong></div></article>)}</div></div></section>
    <section className="journeys-section section-pad"><div className="site-container"><div className="journeys-intro"><p className="eyebrow">Built around the action</p><h2>The right website depends<br /><em>on what needs to happen next.</em></h2><p>Some businesses need a booking. Others need a call, a quote, a purchase or a qualified enquiry. The website should make that next step clear.</p></div><div className="journey-list">{customerJourneys.map(([title, description], index) => <div key={title}><span>{String(index + 1).padStart(2, "0")}</span><h3>{title}</h3><p>{description}</p></div>)}</div></div></section>
    <section className="concepts-section section-pad"><div className="site-container"><div className="concepts-intro"><p className="eyebrow">Explore the concepts</p><h2>Five complete examples<br /><em>of our approach.</em></h2><p>Explore five internal concept projects built to demonstrate how strategy, structure, conversion paths and visual direction can change from one business model to another.</p></div><div className="concept-grid">{projects.map((project, index) => <Link href={`/work/${project.slug}`} className="concept-preview" key={project.slug}><div className="concept-preview-image"><Image src={project.image} alt={`${project.name} concept website cover`} fill sizes="(max-width: 700px) 100vw, 33vw" /><span>Concept Project</span></div><div className="concept-preview-meta"><div><small>0{index + 1} / {project.industry}</small><h3>{project.name}</h3></div><ArrowUpRight aria-hidden="true" /></div></Link>)}</div></div></section>
    <section className="beyond-section section-pad"><div className="site-container"><p className="eyebrow">And beyond</p><h2>Your business does not need<br /><em>to fit a template.</em></h2><p>Not seeing your industry? If your business needs to be found, trusted and chosen online, we can shape the website around how it actually works.</p></div></section>
    <CTASection title="Not sure what your website needs?" body="Start with a clear, practical review of your current digital presence." />
  </main>;
}
