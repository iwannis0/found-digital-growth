...
const goals = ["More calls", "More bookings", "More enquiries", "Better Google visibility", "Professional redesign", "New website", "Other"];
...
export function AuditForm() {
  ...
  if (status === "success") return <div className="form-success" role="status"><CheckCircle2 /><p className="eyebrow">Audit request received</p><h2>Thank you. We will review the information and get back to you as soon as possible.</h2><button className="text-link" onClick={() => setStatus("idle")}>Submit another website</button></div>;
  return <form className="found-form audit-form" onSubmit={submit} noValidate>
    ...
    <div className="form-row"><Field label="Email" name="email" type="email" required error={fields.email?.[0]} /><Field label="Phone" name="phone" type="tel" required error={fields.phone?.[0]} /></div>
    <Field label="Website" name="website" type="url" placeholder="https://" required error={fields.website?.[0]} />
    <div className="form-row"><Field label="Industry" name="industry" required error={fields.industry?.[0]} /><Field label="City" name="city" required error={fields.city?.[0]} /></div>
    <div className="form-row"><SelectField label="Main Goal" value={goal} onChange={setGoal} options={goals} placeholder="Choose your main goal" error={fields.goal?.[0]} /><SelectField label="Approximate Budget" value={budget} onChange={setBudget} options={budgets} placeholder="Choose a range" error={fields.budget?.[0]} /></div>
    <div className="form-field"><Label htmlFor="problem">Current Problem *</Label><Textarea id="problem" name="problem" rows={5} required aria-invalid={Boolean(fields.problem)} /><FieldError value={fields.problem?.[0]} /></div>
...
function SelectField({ label, value, onChange, options, placeholder, error }: { label: string; value: string; onChange: (value: string) => void; options: string[]; placeholder: string; error?: string }) { return <div className="form-field"><Label>{label} *</Label><Select value={value} onValueChange={(next) => onChange(next ?? "")}><SelectTrigger className="found-select" aria-invalid={Boolean(error)}><SelectValue placeholder={placeholder} /></SelectTrigger><SelectContent>{options.map((item) => <SelectItem key={item} value={item}>{item}</SelectItem>)}</SelectContent></Select><FieldError value={error} /></div>; }
